...
// コンストラクタ
CFPSCounter::CFPSCounter(const unsigned int smp) {
	// サンプル数の設定
	SetSampleNum(smp);
...
CFPSCounter::~CFPSCounter() {
	if (m_iCounterFlag == FPSCOUNTER_TIMEGETTIME) timeEndPeriod(1);    // タイマーの精度を戻す
}
...
// FPSを更新
double CFPSCounter::UpdateFPS(const double Def) {
	// 最も古いデータを消去
...
// サンプル数を変更
void CFPSCounter::SetSampleNum(const unsigned int smp) {
	m_uiNum = smp;    // 平均を計算する個数
	m_dwDefTimeLst.resize(smp, 0.0);    // リスト初期化
	m_dwSumTimes = 0;
}